(function(module) {
    ...
    module.directive('aetHasAnyPermission', ['userDetails', 'security', function(userDetails, security) {

        return {
            restrict: 'A',
            transclude: false,
            require: 'aetHasAnyPermission',
            scope: true,
            link: function link(scope, element, attrs, controller) {
                ...
                var permissions = scope.$eval(attrs['aetHasAnyPermission']);

                scope.resolvePermission = function() {
                    var anyAuthorized = _.any(permissions, function(permission) {
                        return security.isAuthorized(permission) === true;
                    })
                    if (anyAuthorized)
                        hasPermission();
                    else {
