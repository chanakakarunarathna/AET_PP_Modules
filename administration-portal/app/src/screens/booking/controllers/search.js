(function(module) {
  ...
  module.controller('SearchBookingController', ['$scope', 'bookingService', '$state', 'alertsService', 'modalService', 'partnerDetails','$filter','$rootScope','previousState', '$stateParams',
    function($scope, bookingService, $state, alertsService, modalService, partnerDetails, $filter, $rootScope, previousState, $stateParams) {    
      ...
      $scope.booking.searchoperator = {key: 'and', value: 'AND'};
      $scope.booking.goodstanding = {key: 'true', value: 'TRUE'};
      ...
      if(previousState.URL!=null && previousState.URL.includes('#/booking/view/') && JSON.parse(sessionStorage.filteredResults) != null){
		...
		var status  = JSON.parse(sessionStorage.bookingParams).status;
		$scope.booking.status = {key: status, value: status.toUpperCase()};
		var vendor  = JSON.parse(sessionStorage.bookingParams).vendor;
		...
		if($stateParams.searchEnabled){
			if($scope.resultDetails.pagination.count==1 && $scope.currentPage != 1){
      ...
      $scope.isValidSearchBookingForm  = function () {
    	  if(($scope.booking!= undefined) && 
			 ...
			 (($scope.booking.confnumber != undefined && $scope.booking.confnumber != "") ||
    		  ...
    		  ($scope.booking.status != undefined && $scope.booking.status.key !="") ||
    		  ($scope.booking.vendor != undefined && $scope.booking.vendor.key != "") || 
