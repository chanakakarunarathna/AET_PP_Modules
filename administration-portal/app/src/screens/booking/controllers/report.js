(function(module) {
  ...
  module.controller('ReportBookingController', ['$scope', 'bookingService', '$state', 'alertsService', 'modalService', 'partnerDetails','$filter','$rootScope','previousState', '$stateParams', 
    function($scope, bookingService, $state, alertsService, modalService, partnerDetails, $filter, $rootScope, previousState, $stateParams) {    
      ...
      $scope.resultReportDetails = {};
      $scope.results = undefined;
      
      $scope.phoneNumberRegex = /^\+?\d{2}[- ]?\d{3}[- ]?\d{5}$/;

      $scope.reset = function() {
    	  	$scope.reportBookingForm.$setPristine();
			$scope.reportBookingForm.$setUntouched();
			$scope.reportBookingForm.$setValidity();
			$scope.reportBookingForm.$showError = false;
			angular.forEach( $scope['reportBookingForm'], function( field, fieldName ) {
				if( fieldName[ 0 ] === '$' ) {
		            return;
		        }
      ...
      $scope.booking.searchoperator = {key: 'and', value: 'AND'};
      $scope.booking.goodstanding = {key: 'false', value: 'FALSE'};
      
      if(sessionStorage.vendorListOptions == undefined){
    	  bookingService.getVendorList().then(function(response) {
    		  sessionStorage.setItem('vendorListOptions', JSON.stringify(response));
    		  $scope.vendorListOptions = JSON.parse(sessionStorage.vendorListOptions);
          });
      }else if(sessionStorage.vendorListOptions != undefined && JSON.parse(sessionStorage.vendorListOptions) != null){
		  $scope.vendorListOptions = JSON.parse(sessionStorage.vendorListOptions);
	  }
      ...
      $scope.retrieveBookingsReport = function(page) {
    	  var bookingObject = {
    	          ...
    	          'vendor'				: ($scope.booking.vendor!=null) ? $scope.booking.vendor.key : "",
        		  'status'				: ($scope.booking.status!=null) ? $scope.booking.status.key : "",
        	      'goodstanding'		: ($scope.booking.goodstanding!=null) ? $scope.booking.goodstanding.key : "",	  
          ...
          bookingService.searchBooking(bookingObject).then(function(response) {
                  $scope.resultReportDetails = response;
                  $scope.filteredReportResults = response.results;
                  $scope.currentPage = page;
                  if($scope.filteredReportResults.length>0){
                	  sessionStorage.setItem('bookingReportParams', JSON.stringify(bookingObject));
                      sessionStorage.setItem('resultReportDetails', JSON.stringify($scope.resultReportDetails));
                      sessionStorage.setItem('filteredReportResults', JSON.stringify($scope.filteredReportResults));
                  }else{
      ...
      $scope.filteredReportResults = [], $scope.currentPage = 0, $scope.numPerPage = 15, $scope.maxSize = 5;

      if(previousState.URL!=null && previousState.URL.includes('#/booking/view/') && JSON.parse(sessionStorage.filteredReportResults) != null){
		$scope.filteredReportResults = JSON.parse(sessionStorage.filteredReportResults);
		$scope.resultReportDetails = JSON.parse(sessionStorage.resultReportDetails);
		$scope.booking = JSON.parse(sessionStorage.bookingReportParams);
		$scope.currentPage = JSON.parse(sessionStorage.bookingReportParams).page +1; 
		var searchoperator  = JSON.parse(sessionStorage.bookingReportParams).searchoperator;
		$scope.booking.searchoperator = {key: searchoperator, value: searchoperator.toUpperCase()};
		var status  = JSON.parse(sessionStorage.bookingReportParams).status;
		$scope.booking.status = {key: status, value: status.toUpperCase()};
  		var vendor  = JSON.parse(sessionStorage.bookingReportParams).vendor;
  		$scope.booking.vendor = {key: vendor, value: vendor.toUpperCase()};
		...
		$scope.booking.goodstanding = {key: 'false', value: 'FALSE'};
		if($stateParams.searchEnabled){
			if($scope.resultReportDetails.pagination.count==1 && $scope.currentPage != 1){
				$scope.currentPage = $scope.currentPage - 1;
			}
			$scope.retrieveBookingsReport($scope.currentPage); 
		}
      }else{
		sessionStorage.filteredReportResults = null;
		sessionStorage.resultReportDetails = null;
