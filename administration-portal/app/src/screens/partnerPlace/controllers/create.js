...
//                setFormValidity(form, 'partnerPlaceLeader', $scope._partnerPlaceErrors);
            ...
            $scope.deleteUniqueRow = function (dto, type) {
                if(type == 'TOPTIPS'){
                	var delModalInstance = modalService.customDeleteModal('Top Tips', 'Are you sure you want to delete this item?');
                	...
                	var delModalInstance = modalService.customDeleteModal('Partner Collection', 'Are you sure you want to delete this item?');
                	delModalInstance.result.then(function() {
            ...
            $scope.manullayAddMapRow = function (key, value) {
            	if ($scope.partnerplace.getToKnow[key]==null) {
            		$scope.partnerplace.getToKnow[key] = value;
                    $scope.manuallyAddedKey = null;
                	$scope.manuallyAddedValue = null;
            	}else{
            		alert("Get To Know Key Exists");
            	}
            };

            $scope.deleteUniqueRowMap = function (key) {
            	var delModalInstance = modalService.customDeleteModal('Get To Know', 'Are you sure you want to delete this item?');
            	delModalInstance.result.then(function() {
            ...
            $scope.addVendorActivityCountRow = function (key, value) {
            	if ($scope.partnerplace.vendorActivityCount[key]==null) {
	            	$scope.partnerplace.vendorActivityCount[key] = value;
	            	$scope.vendorActivityCountKey = null;
	            	$scope.vendorActivityCountValue = null;
            	}else{
            		alert("Vendor Activity Count Key Exists");
            	}
            };

            $scope.deleteVendorActivityCountRow = function (key) {
            	var delModalInstance = modalService.customDeleteModal('Vendor Activity Counts', 'Are you sure you want to delete this item?');
            	delModalInstance.result.then(function() {
            ...
            $scope.refreshPlaces= function(searchName) {
                ...
                _timeout = $timeout(function() {
                    var searchObj = {q: searchName, searchby: 'all'};
