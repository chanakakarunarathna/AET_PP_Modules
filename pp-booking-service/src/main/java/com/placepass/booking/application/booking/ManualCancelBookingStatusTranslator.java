...
@Component
public class ManualCancelBookingStatusTranslator {

    public Status translate(CancelBookingRS cancelBookingCRS, ConnectorPaymentReversalResponse refundResponse) {
        Status status = new Status();
        PlatformStatus platformStatus = null;
        if ((cancelBookingCRS != null) && (cancelBookingCRS.getResultType().getCode() == VendorErrorCode.SUCCESS.getId())) {
        ...
        } else if ((cancelBookingCRS != null) && (cancelBookingCRS.getResultType().getCode() == VendorErrorCode.CANCEL_UNKNOWN.getId())) {
            if (refundResponse == null
                    || PaymentProcessStatus.PAYMENT_REVERSAL_SUCCESS == refundResponse.getPaymentStatus()) {
                platformStatus = PlatformStatus.SUCCESS;
        ...
        } else if ((cancelBookingCRS != null) && (cancelBookingCRS.getResultType().getCode() == VendorErrorCode.VENDOR_CONNECTION_TIMEOUT_ERROR.getId()
