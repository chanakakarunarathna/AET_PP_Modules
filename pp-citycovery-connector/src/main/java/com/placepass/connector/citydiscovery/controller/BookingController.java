...
import com.placepass.connector.citydiscovery.application.booking.BookingService;
...
public class BookingController extends HTTPResponseHandler {
    ...
    @ResponseBody
    public GetProductPriceRS getProductPrice(@Valid @RequestBody GetProductPriceRQ productPriceRQ,
            HttpServletResponse response) {

        logger.info("Received Product Price Request");
        return bookingService.getProductPrice(productPriceRQ);
    }
    ...
    @ResponseBody
    public BookingVoucherRS getVoucherDetails(@Valid @RequestBody BookingVoucherRQ bookingVoucherRQ,
            HttpServletResponse response) {
    ...
    @ResponseBody
    public CancelBookingRS cancelBooking(@Valid @RequestBody CancelBookingRQ cancelBookingRQ,
            HttpServletResponse response) {
    ...
    @ApiOperation(value = "getBookingStatus", nickname = "getBookingStatus")
    // Documenting status codes (these are samples)
    @ApiResponses(value = {@ApiResponse(code = 200, message = "Success", response = BookingStatusRS.class),
        @ApiResponse(code = 401, message = "Unauthorized"), @ApiResponse(code = 403, message = "Forbidden"),
        @ApiResponse(code = 404, message = "Not Found"), @ApiResponse(code = 500, message = "Failure"),
        @ApiResponse(code = 400, message = "Bad Request")})
    @RequestMapping(value = "/booking/status", method = RequestMethod.POST)
    @ResponseBody
    public BookingStatusRS getBookingStatus(@Valid @RequestBody BookingStatusRQ bookingStatusRQ,
            HttpServletResponse response) {

        logger.info("Received Booking Status Request");
        return bookingService.getBookingStatus(bookingStatusRQ);
    }
